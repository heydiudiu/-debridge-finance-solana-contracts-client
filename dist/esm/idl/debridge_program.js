export const IDL = {
    ...
    instructions: [
        ...
        {
            name: "initOrUpdateExternalCallStorage",
            docs: [
                "Initializes or updates the external call storage",
                "",
                "This function checks if the external call storage already exists. If not, it creates a new account for the storage",
                "using the provided key, chain ID, and bump. If the storage exists, it updates the existing storage with the provided",
                "raw instructions at the specified offset.",
                "",
                "The function uses an accumulation mechanism, ensuring that updates are only performed if the storage is in an",
                "accumulation state, and the new data does not exceed the storage capacity.",
                "",
                "This function supports handling external data that exceeds the transaction size limit by allowing incremental updates.",
                "",
                "# Arguments",
                "* `ctx` - The context holding account and bump information.",
                "* `external_call_len` - The total length of the external call data.",
                "* `chain_id` - The ID of the blockchain network where the call originates or targets.",
                "* `storage_key` - A unique identifier used for the storage, typically a submission ID.",
                "* `external_instructions_offset` - The offset within the storage to start writing the new instructions.",
                "* `raw_instructions` - The actual instructions to be stored.",
                "",
                "# Errors",
                "Returns an error if the storage cannot be initialized, if the update is not permissible by the current state,",
                "or if the provided data exceeds the storage size limit.",
                "",
                "# Roles",
                "👤 Claimer - The primary role interacting with this function, responsible for initializing or updating the storage.",
            ],
            accounts: [
                {
                    name: "externalCallStorage",
                    isMut: true,
                    isSigner: false,
                    docs: ["Storage for external call instructions", "", "The key is checked after calculating the Submission id"],
                },
                {
                    name: "externalCallMeta",
                    isMut: true,
